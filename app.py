...
def send_distress_email(to_email, user_name, user_email, triggering_text):
    ...
    message_body = (
        ...
        "The user may be in emotional distress. Please contact them or local support immediately."
    )
    message = f"Subject: {subject}\n\n{message_body}"
    try:
        server = smtplib.SMTP("smtp.gmail.com", 587)
...
def new_chat():
    ...
    save_chat_message(email,chat_name,"bot","Hi! I'm Manas, your AI mental health assistant.")
...
def chat_message():
    ...
    distress_detected=any(k in user_message.lower() for k in help_keywords) or TextBlob(user_message).sentiment.polarity<-0.5
...
"""
    ...
    if distress_detected:
        send_distress_email("manasemergencysos@gmail.com",name,email,user_message)
